...
describe('College appointment end-to-end flow', () => {
    test('full user flow: register/login, set availability, book, cancel, check', async () => {
      ...
      const profReg = await request(app).post('/auth/register').send({
        name: 'Prof P1',
      ...
      const profId = profReg.body.user.id;
        ...
        const s1Reg = await request(app).post('/auth/register').send({
        name: 'Student A1',
      ...
      expect(appt1.studentId).toBe(s1Id.toString() || s1Id);
  
      const s2Reg = await request(app).post('/auth/register').send({
        name: 'Student A2',
      ...
      expect(appt2.studentId).toBe(s2Id.toString() || s2Id);
  
      const cancel = await request(app)
        .post(`/appointments/${appt1.id}/cancel`)
        .set('Authorization', `Bearer ${profToken}`)
        .send();
